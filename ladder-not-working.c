...
/********************* BEGIN FORCE CALCULATION ***********************/
	...
	//	f[i][2]+=ziggurat(omp_get_thread_num())*randFmult;
		...
		f[i][0]=ziggurat(0)*randFmult;
		f[i][1]=ziggurat(0)*randFmult;
		f[i][2]=ziggurat(0)*randFmult;
...
//		f[i][2]+=randn()*randFmult;
	...
	// Calculate forces from intra bonds
	...
	// Calculate forces from inter bonds
...
/****************** END OF FORCE CALCULATION **************************/
	...
	for (i=0; i<2*N; i++)  { 
			...
			v[i][0] += (f[i][0]*halfdtMass)* halfdtgammanorm;
			v[i][1] += (f[i][1]*halfdtMass)* halfdtgammanorm;
			v[i][2] += (f[i][2]*halfdtMass)* halfdtgammanorm;
